...
5. Check for Non-Substantive ('NONE'): Is the utterance purely filler ("Uhm", "Let me see", "Oops") or lacks any recognizable cognitive process mining structure?

Instructions for Extracting Segments (Quotes):
- The input transcript is in Dutch.
- For each classified label (except NONE when other labels are present), you MUST extract the exact word-for-word quote (`exact_quote`) from the transcript.
...
- If the entire transcript consists of filler words, or has no substantive cognitive structures (DK, PK, CK, DOM), you MUST return a single segment with label 'NONE' and the `exact_quote` matching the entire transcript text.
- Provide a 'confidence_score' between 0.0 and 1.0 for each annotation. If you are highly uncertain, prioritize 'NONE' and a low confidence score."""
